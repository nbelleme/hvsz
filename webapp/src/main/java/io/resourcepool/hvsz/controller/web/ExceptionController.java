...
public class ExceptionController {
  ...
  public String humanAlreadyDead(HumanIsDeadException ex) {
    if (ex.getSupplyZone() != null) {
      return "redirect:/food-supply/" + ex.getSupplyZone().getId();
    }
    if (ex.getSafeZone() != null) {
      return "redirect:/safe-zone/" + ex.getSafeZone().getId();
    }
    if (ex.getZombieZone() != null) {
      return "redirect:/zombie-hq";
    }
